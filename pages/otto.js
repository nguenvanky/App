...
<body>
    <!-- Menu Button (Pin) ở góc trái -->
    <button class="menu-button" onclick="toggleMenu()" title="Menu">📌</button>
    <button class="menu-button menu-ai" onclick="forceAiStart()" title="Kích hoạt AI">⚡</button>
    
    <!-- Menu Panel -->
    <div id="menu-panel" class="menu-panel">
        <div class="menu-item" onclick="scanQRCode(); toggleMenu();">
            📷 <span data-i18n="otto.scan_qr">Scan QR Code</span>
        </div>
        <div class="menu-item" onclick="showSavedDevices(); toggleMenu();">
            📋 <span data-i18n="otto.saved_devices">Saved Devices</span>
        </div>
        <div class="menu-item" onclick="showQrCodeOnLCD(); toggleMenu();">
            📺 <span data-i18n="otto.show_qr_lcd">Show QR on LCD</span>
        </div>
        <div class="menu-item" onclick="toggleVoiceControl(); toggleMenu();">
            🎤 <span id="menu-voice-text" data-i18n="otto.voice_start">Start Voice Control</span>
        </div>
        ...
        <div class="menu-item" onclick="window.location.href='/servo-calibration'">
            ⚙️ <span data-i18n="otto.open_servo_calibration">Servo Calibration</span>
        </div>
    ...
    <div id="qr-scanner" style="display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 20px; border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.3); z-index: 2000; max-width: 90%; max-height: 90%;">
        <div style="text-align: right; margin-bottom: 10px;">
            <button onclick="stopQRScanner()" style="background: #dc3545; color: white; border: none; padding: 5px 15px; border-radius: 6px; cursor: pointer;">✕</button>
        </div>
    ...
    <div id="saved-devices-list" style="display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 20px; border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.3); z-index: 2000; max-width: 90%; max-height: 80%; overflow-y: auto;">
        <div style="text-align: right; margin-bottom: 10px;">
            <button onclick="document.getElementById('saved-devices-list').style.display='none'" style="background: #dc3545; color: white; border: none; padding: 5px 15px; border-radius: 6px; cursor: pointer;">✕</button>
        </div>
        <h3 style="margin-bottom: 15px;">📋 <span data-i18n="otto.saved_devices">Saved Devices</span></h3>
    ...
    <div class="container">
        ...
        <h1 data-i18n="otto.title">🤖 Otto Robot Control</h1>
        ...
        <div class="control-section" style="background: linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%); border-left: 4px solid #4caf50; padding: 15px; border-radius: 12px;">
            <h2 class="section-title" data-i18n="otto.direction" style="color: #2e7d32; border-color: #4caf50;">🎮 Direction Control (Gamepad)</h2>
            <div class="direction-grid">
                <div></div>
                <button class="btn btn-direction" onclick="sendCommand('walk_forward')" data-i18n="otto.forward" style="background: #4caf50;">⬆️ Forward</button>
                <div></div>
                <button class="btn btn-direction" onclick="sendCommand('turn_left')" data-i18n="otto.left" style="background: #2196f3;">⬅️ Left</button>
                <button class="btn btn-direction btn stop" onclick="sendCommand('stop')" data-i18n="otto.stop">⏹️ Stop</button>
                <button class="btn btn-direction" onclick="sendCommand('turn_right')" data-i18n="otto.right" style="background: #2196f3;">➡️ Right</button>
                <div></div>
                <button class="btn btn-direction" onclick="sendCommand('walk_backward')" data-i18n="otto.backward" style="background: #4caf50;">⬇️ Backward</button>
                <div></div>
            </div>
        </div>
        ...
        <div class="control-section volume-section">
            <h2 class="section-title collapsible collapsed" onclick="toggleSection(this)">🔊 Volume / Âm lượng</h2>
        ...
        <div class="control-section">
            <h2 class="section-title collapsible" onclick="toggleSection(this)" data-i18n="otto.dance">💃 Dance Moves</h2>
        ...
        <div class="control-section">
            <h2 class="section-title collapsible" onclick="toggleSection(this)" data-i18n="otto.hands">👋 Hand Actions</h2>
            <div class="collapsible-content">
                <div class="control-grid">
        ...
        <div class="control-section">
            <h2 class="section-title collapsible" onclick="toggleSection(this)" data-i18n="otto.emoji">😊 Emoji Control</h2>
            <div class="collapsible-content">
                <div class="emoji-grid">
                    <button class="btn emoji-btn" onclick="setEmoji('happy')" data-i18n="otto.emoji.happy">😊 Happy</button>
                    <button class="btn emoji-btn" onclick="setEmoji('sad')" data-i18n="otto.emoji.sad">😢 Sad</button>
                    <button class="btn emoji-btn" onclick="setEmoji('angry')" data-i18n="otto.emoji.angry">😠 Angry</button>
                    <button class="btn emoji-btn" onclick="setEmoji('surprised')" data-i18n="otto.emoji.surprised">😲 Surprised</button>
                    <button class="btn emoji-btn" onclick="setEmoji('thinking')" data-i18n="otto.emoji.thinking">🤔 Thinking</button>
                    <button class="btn emoji-btn" onclick="setEmoji('neutral')" data-i18n="otto.emoji.neutral">😐 Neutral</button>
                </div>
            </div>
        </div>
        
        <div class="control-section" style="background: linear-gradient(135deg, #fff5e6 0%, #ffe6cc 100%); border-left: 4px solid #ff9800;">
            <h2 class="section-title" data-i18n="otto.servo_calibration">⚙️ Servo Calibration (Điều Chỉnh Góc Servo)</h2>
    ...
    <script>
        ...
        function forceAiStart() {
            ...
            .then(d => {
                if (d.success) {
                    showStatus('AI đã được kích hoạt!', false);
                } else {
                    showStatus('Lỗi: ' + (d.error || 'Unknown error'), true);
                }
            })
            .catch(e => showStatus('Error: ' + e, true));
        }
        ...
        async function setDeviceVolume(value) {
            ...
            try {
                ...
                if (typeof showStatus === 'function') {
                    showStatus('Không thể cập nhật âm lượng: ' + err.message, true);
                }
            }
        }
        ...
        async function showQrCodeOnLCD() {
            try {
                showStatus('Đang hiển thị QR code lên màn hình LCD...', false);
                ...
                if (!res.ok) {
                    ...
                    showStatus('Lỗi khi hiển thị QR code: ' + res.status, true);
                    return;
                }
                const data = await res.json();
                if (data.success) {
                    showStatus('✅ QR code đã được hiển thị trên màn hình LCD!', false);
                    ...
                    showStatus('Lỗi: ' + errorMsg, true);
                }
            } catch (e) {
                console.error('Error showing QR code:', e.message);
                showStatus('Lỗi kết nối: ' + e.message, true);
            }
        }
    ...
    <script>
        ...
        const translations = {
            'zh-CN': {
                'otto.title': '🤖 Otto 机器人控制',
                'otto.status.idle': '状态: 空闲',
                'otto.status.moving': '状态: 移动中',
                'otto.direction': '方向控制',
                'otto.forward': '⬆️ 前进',
                'otto.left': '⬅️ 左转',
                'otto.right': '➡️ 右转',
                ...
                'otto.shake_leg': '摇腿',
                'otto.updown': '上下',
                'otto.tiptoe_swing': '踮脚摇摆',
                'otto.jitter': '抖动',
                'otto.ascending_turn': '上升转身',
                ...
                'otto.emoji.sad': '😢 难过',
                'otto.emoji.angry': '😠 生气',
                'otto.emoji.surprised': '😲 惊讶',
                'otto.emoji.thinking': '🤔 思考',
                'otto.emoji.neutral': '😐 中性',
                'otto.voice_control': '🎤 语音控制 (语音指令)',
                'otto.voice_start': '🎤 开始语音控制',
                'otto.voice_stop': '⏹️ 停止录音',
                'otto.voice_hint': '点击按钮并说话以控制Otto机器人。AI会理解您的指令并自动控制Otto。示例: "Otto 前进", "Otto 跳跃", "Otto 停止"',
                'otto.voice_status_recording': '🎤 正在录音... 请说话！',
                'otto.voice_status_processing': '🤔 处理中... 正在发送到AI...',
                'otto.voice_status_sent': '✅ 指令已发送！AI正在处理...'
            },
            'zh-TW': {
                'otto.title': '🤖 Otto 機器人控制',
                'otto.status.idle': '狀態: 閒置',
                'otto.status.moving': '狀態: 移動中',
                'otto.direction': '方向控制',
                'otto.forward': '⬆️ 前進',
                'otto.left': '⬅️ 左轉',
                'otto.right': '➡️ 右轉',
                ...
                'otto.shake_leg': '搖腿',
                'otto.updown': '上下',
                'otto.tiptoe_swing': '踮腳搖擺',
                'otto.jitter': '抖動',
                'otto.ascending_turn': '上升轉身',
                'otto.crusaito': 'Crusaito',
                'otto.flapping': '拍打',
                'otto.hands': '手部動作',
                'otto.hands_up': '舉手',
                ...
                'otto.emoji.sad': '😢 難過',
                'otto.emoji.angry': '😠 生氣',
                'otto.emoji.surprised': '😲 驚訝',
                'otto.emoji.thinking': '🤔 思考',
                'otto.emoji.neutral': '😐 中性',
                'otto.voice_control': '🎤 語音控制 (語音指令)',
                'otto.voice_start': '🎤 開始語音控制',
                'otto.voice_stop': '⏹️ 停止錄音',
                'otto.voice_hint': '點擊按鈕並說話以控制Otto機器人。AI會理解您的指令並自動控制Otto。範例: "Otto 前進", "Otto 跳躍", "Otto 停止"',
                'otto.voice_status_recording': '🎤 正在錄音... 請說話！',
                'otto.voice_status_processing': '🤔 處理中... 正在發送到AI...',
                'otto.voice_status_sent': '✅ 指令已發送！AI正在處理...'
            },
            'en-US': {
                'otto.title': '🤖 Otto Robot Control',
                'otto.status.idle': 'Status: Idle',
                'otto.status.moving': 'Status: Moving',
                'otto.direction': 'Direction Control',
                'otto.forward': '⬆️ Forward',
                'otto.left': '⬅️ Left',
                'otto.right': '➡️ Right',
                'otto.backward': '⬇️ Backward',
                'otto.stop': '⏹️ Stop',
                ...
                'otto.emoji.happy': '😊 Happy',
                'otto.emoji.sad': '😢 Sad',
                'otto.emoji.angry': '😠 Angry',
                'otto.emoji.surprised': '😲 Surprised',
                'otto.emoji.thinking': '🤔 Thinking',
                'otto.emoji.neutral': '😐 Neutral',
                'otto.voice_control': '🎤 Voice Control (Voice Command)',
                'otto.voice_start': '🎤 Start Voice Control',
                'otto.voice_stop': '⏹️ Stop Recording',
                ...
                'otto.voice_status_recording': '🎤 Recording... Speak now!',
                'otto.voice_status_processing': '🤔 Processing... Sending to AI...',
                'otto.voice_status_sent': '✅ Command sent! AI is processing...'
            },
            'vi-VN': {
                'otto.title': '🤖 Điều Khiển Robot Otto',
                'otto.status.idle': 'Trạng thái: Nhàn rỗi',
                'otto.status.moving': 'Trạng thái: Đang di chuyển',
                'otto.direction': 'Điều Khiển Hướng',
                'otto.forward': '⬆️ Tiến',
                'otto.left': '⬅️ Trái',
                'otto.right': '➡️ Phải',
                'otto.backward': '⬇️ Lùi',
                'otto.stop': '⏹️ Dừng',
                'otto.dance': 'Động Tác Nhảy',
                'otto.jump': 'Nhảy',
                'otto.swing': 'Đu Đưa',
                'otto.moonwalk': 'Moonwalk',
                'otto.bend': 'Cúi',
                'otto.shake_leg': 'Lắc Chân',
                'otto.updown': 'Lên & Xuống',
                'otto.tiptoe_swing': 'Đu Đưa Nhón Chân',
                'otto.jitter': 'Rung',
                'otto.ascending_turn': 'Xoay Lên',
                'otto.crusaito': 'Crusaito',
                'otto.flapping': 'Vỗ',
                'otto.hands': 'Động Tác Tay',
                'otto.hands_up': 'Giơ Tay',
                'otto.hands_down': 'Hạ Tay',
                'otto.hand_wave': 'Vẫy Tay',
                'otto.emoji': 'Điều Khiển Emoji',
                'otto.emoji.happy': '😊 Vui',
                'otto.emoji.sad': '😢 Buồn',
                'otto.emoji.angry': '😠 Giận',
                'otto.emoji.surprised': '😲 Ngạc Nhiên',
                'otto.emoji.thinking': '🤔 Suy Nghĩ',
                'otto.emoji.neutral': '😐 Trung Tính',
                'otto.voice_control': '🎤 Điều Khiển Giọng Nói (Lệnh Giọng Nói)',
                'otto.voice_start': '🎤 Bắt Đầu Điều Khiển Giọng Nói',
                'otto.voice_stop': '⏹️ Dừng Ghi Âm',
                'otto.voice_hint': 'Nhấn nút và nói để điều khiển robot Otto. AI sẽ hiểu lệnh của bạn và tự động điều khiển Otto. Ví dụ: "Otto đi về phía trước", "Otto nhảy", "Otto dừng lại"',
                'otto.voice_status_recording': '🎤 Đang ghi âm... Vui lòng nói!',
                'otto.voice_status_processing': '🤔 Đang xử lý... Đang gửi đến AI...',
                'otto.voice_status_sent': '✅ Lệnh đã được gửi! AI đang xử lý...'
            },
            'zh-CN': {
                'otto.title': '🤖 Otto 机器人控制',
                'otto.status.idle': '状态: 空闲',
                ...
                'otto.voice_status_processing': '🤔 处理中... 正在发送到AI...',
                'otto.voice_status_sent': '✅ 指令已发送！AI正在处理...',
                'otto.device_discovery': '🔍 设备发现',
                'otto.scan_qr': '📷 扫描二维码',
                'otto.saved_devices': '📋 已保存设备',
                'otto.stop_scan': '停止扫描',
                'otto.no_device_found': '未找到设备',
                'otto.connect': '连接',
                'otto.disconnect': '断开连接',
                'otto.remove': '删除',
                'otto.online': '在线',
                'otto.offline': '离线',
                'otto.show_qr_lcd': '📺 在LCD显示QR码'
            },
            'zh-TW': {
                'otto.device_discovery': '🔍 裝置發現',
                'otto.scan_qr': '📷 掃描 QR Code',
                'otto.saved_devices': '📋 已儲存裝置',
                'otto.stop_scan': '停止掃描',
                'otto.no_device_found': '找不到裝置',
                'otto.connect': '連接',
                'otto.disconnect': '斷開連接',
                'otto.remove': '刪除',
                'otto.online': '線上',
                'otto.offline': '離線',
                'otto.show_qr_lcd': '📺 在LCD顯示QR碼'
            },
            'vi-VN': {
                'otto.device_discovery': '🔍 Tìm Thiết Bị',
                'otto.scan_qr': '📷 Quét QR Code',
                'otto.saved_devices': '📋 Thiết Bị Đã Lưu',
                'otto.stop_scan': 'Dừng Quét',
                'otto.no_device_found': 'Không tìm thấy thiết bị',
                'otto.connect': 'Kết Nối',
                'otto.disconnect': 'Ngắt Kết Nối',
                'otto.remove': 'Xóa',
                'otto.online': 'Trực Tuyến',
                'otto.offline': 'Ngoại Tuyến',
                'otto.show_qr_lcd': '📺 Hiển Thị QR Trên LCD'
            },
            'en-US': {
                'otto.show_qr_lcd': '📺 Show QR on LCD'
            },
            'ja-JP': {
                'otto.show_qr_lcd': '📺 LCDにQRコードを表示'
            },
            'ko-KR': {
                'otto.show_qr_lcd': '📺 LCD에 QR 표시'
            },
            'th-TH': {
                'otto.show_qr_lcd': '📺 แสดง QR บน LCD'
            }
        };
        ...
        if (isSpeechSupported) {
            ...
            recognition.onstart = function() {
                ...
                if (statusEl) {
                    ...
                    const t = translations[lang] || translations['en-US'];
                    statusEl.textContent = t['otto.voice_status_recording'] || '🎤 Recording... Speak now!';
                }
                // Update menu text
                ...
                if (menuVoiceText) {
                    const lang = getCurrentLanguage();
                    const t = translations[lang] || translations['en-US'];
                    menuVoiceText.textContent = t['otto.voice_stop'] || '⏹️ Stop Recording';
                }
            };
        ...
        function updateVoiceButton(recording) {
            ...
            if (recording) {
                ...
                btn.textContent = t['otto.voice_stop'] || '⏹️ Stop Recording';
            } else {
                btn.classList.remove('voice-recording');
                btn.textContent = t['otto.voice_start'] || '🎤 Start Voice Control';
            }
        }
        ...
        async function sendVoiceCommandToAI(text) {
            ...
            if (statusEl) {
                ...
                const lang = getCurrentLanguage();
                const t = translations[lang] || translations['en-US'];
                statusEl.textContent = t['otto.voice_status_processing'] || '🤔 Processing... Sending to AI...';
            }
            ...
            try {
                ...
                if (data.success && statusEl) {
                    ...
                    const lang = getCurrentLanguage();
                    const t = translations[lang] || translations['en-US'];
                    statusEl.textContent = t['otto.voice_status_sent'] || '✅ Command sent! AI is processing...';
                    setTimeout(() => {
                        if (statusEl) {
                            statusEl.textContent = '';
                            statusEl.style.display = 'none';
                    ...
                    statusEl.className = '';
                    statusEl.textContent = '❌ Error: ' + (data.message || 'Failed to send command');
                }
            } catch (e) {
                statusEl.className = '';
        ...
        async function setDeviceVolume(value) {
            ...
            try {
                ...
                console.error('setDeviceVolume error', err);
                notifyStatus('Không thể cập nhật âm lượng: ' + err.message, true);
            }
        }
        ...
        async function forceAiStart() {
            if (forceAiBusy) {
                notifyStatus('Đang kích hoạt AI...', false);
                return;
            }
            forceAiBusy = true;
            notifyStatus('Đang kích hoạt AI...', false);
            try {
                ...
                notifyStatus('AI đã được kích hoạt!', false);
            } catch (err) {
                console.error('forceAiStart error', err);
                notifyStatus('Không thể kích hoạt AI: ' + err.message, true);
            } finally {
        ...
        function installPWA() {
            ...
            if (isAndroid) {
                ...
                const instructions = 
                    ...
                    '💡 Ứng dụng sẽ lưu trực tiếp trang Otto Control này, không cần qua GitHub Pages.';
                alert(instructions);
            } else {
